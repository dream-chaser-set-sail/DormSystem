...
function loadData(){
    adminapi.list(adminQuery).then(function (obj){
...
function handleDelete(val){
    adminapi.deleteById(val).then(function (obj){
...
function submitUser(){
    switch (isAddorUpdata.value){
        case 2:
            adminapi.add(form.value).then(function (obj){
                myUtils.open(obj.code, obj.msg)
...
function deleteAll(){
    myUtils.openWindow("批量删除", "你确定要删除吗？", function (){
        adminapi.deleteAll(ids.value).then(function (obj){
            myUtils.open(obj.code, obj.msg)
            loadData()
        })
    })
...
<template>
    <el-card>
        <template #header>
            <div class="header">
                <el-button type="primary" @click="showAddDialog" plain>添加管理</el-button>
        ...
        <el-table :data="tableData" ref="multipleTableRef" @selection-change="handleSelectionChange" style="width: 100%">
            ...
            <el-table-column align="center" header-align = 'center' prop="gender" label="姓别" />
