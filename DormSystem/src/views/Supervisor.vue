...
import {supervisorapi} from "@/api/supervisor.js";
...
const supervisorQuery = reactive({
    name : '',
    page : 1,
    limit : 5
})
...
function loadData(){
    supervisorapi.list(supervisorQuery).then(function (obj){
...
function handleDelete(val){
    supervisorapi.deleteById(val).then(function (obj){
...
function handleEdit(val){
    dialogFormVisible.value = true
    supervisorapi.selectById(val).then(function (obj){
        title.value = "修改信息"
...
function submitUser(){
    switch (isAddorUpdata.value){
        case 1:
            supervisorapi.update(form.value).then(function (obj){
                myUtils.open(obj.code, obj.msg)
            ...
            break
        case 2:
            supervisorapi.add(form.value).then(function (obj){
                myUtils.open(obj.code, obj.msg)
                dialogFormVisible.value = false
                loadData()
            })
...
function deleteAll(){
    myUtils.openWindow("批量删除", "你确定要删除吗？", function (){
        supervisorapi.deleteAll(ids.value).then(function (obj){
            myUtils.open(obj.code, obj.msg)
            loadData()
        })
    })
...
function selBuildingName(){
    supervisorapi.selBuildingName().then(function (obj){
        buildingName.value = obj.data
    })
}
...
<template>
    <el-card>
        <template #header>
            <div class="header">
                <el-button type="primary" @click="showAddDialog" plain>添加宿管</el-button>
                <el-button type="danger" @click="deleteAll" plain>批量删除</el-button>
            </div>
        </template>
        <el-form :inline="true">
            <el-form-item label="名字" style="width: 200px">
                <el-input v-model="supervisorQuery.name" placeholder="请输入名字" clearable/>
            </el-form-item>
            <el-form-item label="楼号">
                <el-select
                    v-model="supervisorQuery.buildingName"
                    placeholder="选择楼号"
                    clearable
                    style="width: 120px"
                >
            ...
            <el-form-item label="更新时间">
                <el-date-picker
                        v-model="supervisorQuery.createTime"
                        type="datetime"
                        value-format="YYYY-MM-DD HH:mm:ss"
                        placeholder="开始时间"
                />
                <el-date-picker
                        v-model="supervisorQuery.updateTime"
                        type="datetime"
                        value-format="YYYY-MM-DD HH:mm:ss"
                        placeholder="结束时间"
                />
        ...
        <el-table :data="tableData" ref="multipleTableRef" @selection-change="handleSelectionChange" style="width: 100%">
            ...
            <el-table-column align="center" header-align = 'center' prop="buildingName" label="负责的宿舍楼" />
        ...
        <el-pagination
                background
                :total="tatal"
                v-model:current-page="supervisorQuery.page"
                v-model:page-size="supervisorQuery.limit"
